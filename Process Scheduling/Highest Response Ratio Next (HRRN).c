// 4. Highest Response Ratio Next (HRRN) Algorithm - Nilanjana Thakur (205121064)
...
void hrrn(process* list)
{
	int i,j,curr,oldcurr;
	int start_time,end_time;
	int wait=0;
	double sum_wait=0.0, sum_ta=0.0;
	curr=maxArrived(list,list[0].arrival_time);
	...
	for(i=0;i<n;i++)
	{
		...
		oldcurr=curr;
		curr=maxArrived(list,list[curr].completion_time);
		if(curr==-1)
		{
			curr=maxNext(list);
			curr=maxArrived(list, list[curr].arrival_time);
		}
	}
...
int maxArrived(process *list, int time) // finds the process with maximum response ratio among the list of all arrived processes which haven't already executed
{
	int i,maxpos=-1;
	float max=-999999;
	for(i=0;i<n;i++)
	{
		list[i].rr=(time-list[i].arrival_time+list[i].burst_time)/(list[i].burst_time);
		if(list[i].completion_time==0 && list[i].arrival_time<=time && list[i].rr>max)
		{
			max=list[i].rr;
			maxpos=i;
		}
